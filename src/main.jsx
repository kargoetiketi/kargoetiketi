...
function Barcode({value, className='', height=48, width=1.85}){ const ref=useRef(null); useEffect(()=>{if(ref.current) JsBarcode(ref.current, value, {format:'CODE128', displayValue:true, fontSize:13, height, width, margin:0, textMargin:3});},[value,height,width]); return <svg className={'barcode '+className} ref={ref}/> }
...
function App(){
 ...
 return <>
  <div className="app"><aside className="panel">
   ...
   {region==='TR'&&<label>Ürünler <small>(kısa ürün notu aynı etiketin altında görünür)</small><textarea rows="4" value={form.products} onChange={e=>update('products',e.target.value)} placeholder="1. Ürün adı / adet / renk..."/></label>}
  ...
  </aside><main className="preview"><div ref={labelRef}><Label region={region} c={c} d={data}/></div></main></div>
 </>
}
...
function T1({c,d}){return <><header><Logo c={c}/><div><h2>{c.name}</h2><p>{niceDate(d.date)} / PAKET / Kg: {d.kg}</p></div></header><div className="box slim"><Row title="GÖND.">{d.sender}</Row><Row title="ALICI">{d.recipient}<br/>{d.address}</Row></div><Barcode value={d.track} className="wideBar megaBar" height={76} width={1.45}/><div className="codeLine">{d.track}</div><div className="district">{d.district}</div></>}
function T2({c,d}){return <><header><Logo c={c}/><div><h2>{c.name}</h2><p>{niceDate(d.date)} / Kg: {d.kg}</p></div></header><div className="box"><Row title="GÖNDEREN">{d.sender}</Row><Row title="ALICI">{d.recipient}<br/>{d.address}</Row></div><Barcode value={d.track} className="wideBar megaBar" height={76} width={1.45}/><div className="district small">{d.district}</div><QR value={d.track}/></>}
...
function T4({c,d}){return <><header><Logo c={c}/><div><h2>{c.name}</h2><p>KARGO ETİKETİ</p></div></header><div className="box"><Row title="GÖNDERİCİ">{d.sender}</Row><Row title="ALICI">{d.recipient}<br/>{d.address}</Row></div><Barcode value={d.track} className="wideBar megaBar" height={78} width={1.45}/><div className="district">{d.district}</div><div className="foot">{niceDate(d.date)} • Kg/Desi: {d.kg}</div></>}
...
function T6({c,d}){return <><div className="marketHead">Kargo şirketinin dikkatine, bu bir trendyol.com gönderisidir.</div><Logo c={c} big/><div className="trendyGrid"><div className="card"><h3>Alıcı Bilgileri</h3><p><b>Ad Soyad</b>: {d.recipient}</p><p><b>Adres</b>: {d.address}</p></div><div className="card barkodCard"><h3>Kargo Barkodu</h3><Barcode value={d.track} height={58} width={1.35}/></div></div>{d.products.trim()&&<div className="inlineProducts"><b>Ürün Bilgileri</b>{d.products.split('\n').filter(Boolean).slice(0,3).map((p,i)=><p key={i}>{i+1}. {p}</p>)}</div>}<div className="district trendyDistrict">{d.district}</div><div className="foot">Sipariş No: {rnd(10)} • Kg/Desi: {d.kg} • {niceDate(d.date)}</div></>}
function T7({c,d}){return <><header><Logo c={c}/><div><h2>{c.name}</h2><p>{niceDate(d.date)} / STANDART</p></div></header><div className="box"><Row title="GÖN.">{d.sender}</Row><Row title="ALICI">{d.recipient}<br/>{d.address}</Row></div><Barcode value={d.track} className="wideBar megaBar" height={78} width={1.45}/><div className="district">{d.district}</div></>}
